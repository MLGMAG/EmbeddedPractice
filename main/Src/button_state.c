...
UAL_BUTTON_STATE_t UAL_BUTTON_STATE_GetState() { return state; }
...
void UAL_BUTTON_STATE_UpdateState(uint8_t value) {
	...
	if (state == UAL_BUTTON_STATE_PRESSED &&
		recording_count < MAX_RECORDING_COUNT) {
	...
	if (state == UAL_BUTTON_STATE_HELD &&
		recording_count < MAX_RECORDING_COUNT) {
		recordings[recording_count] = value;
		recording_count++;
	}
