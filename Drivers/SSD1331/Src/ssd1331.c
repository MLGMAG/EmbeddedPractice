...
extern SPI_HandleTypeDef SSD1331_SPI;

static uint16_t H24_RGB565(uint8_t reverse, uint32_t color24) {
...
static void LCD_Command(uint8_t cmd) {
	HAL_GPIO_WritePin(SSD1331_CS_PORT, SSD1331_CS_PIN, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(SSD1331_DC_PORT, SSD1331_DC_PIN, GPIO_PIN_RESET);
	HAL_SPI_Transmit(&SSD1331_SPI, &cmd, 1, 1000);
	HAL_GPIO_WritePin(SSD1331_CS_PORT, SSD1331_CS_PIN, GPIO_PIN_SET);
}

static void LCD_Data_8(uint8_t data) {
	HAL_GPIO_WritePin(SSD1331_CS_PORT, SSD1331_CS_PIN, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(SSD1331_DC_PORT, SSD1331_DC_PIN, GPIO_PIN_SET);
	HAL_SPI_Transmit(&SSD1331_SPI, &data, 1, 1000);
	HAL_GPIO_WritePin(SSD1331_CS_PORT, SSD1331_CS_PIN, GPIO_PIN_SET);
}

void SSD1331_Frame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
		uint32_t outColor, uint32_t fillColor) {
...
void SSD1331_Rect_Fill(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
		uint32_t color24) {
	SSD1331_Frame(x, y, x + w - 1, y + h - 1, color24, color24);
}

void SSD1331_Line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
	...
	int error2 = 0;
	for (;;) {
		SSD1331_Rect_Fill(x1, y1, size, size, color24);
		if (x1 == x2 && y1 == y2)
			break;
		error2 = error * 2;
		if (error2 > -deltaY) {
...
void SSD1331_Rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t size,
		uint32_t color24) {
	SSD1331_Line(x, y, x + w, y, size, color24);
	SSD1331_Line(x, y + h, x + w, y + h, size, color24);
	SSD1331_Line(x, y, x, y + h, size, color24);
	SSD1331_Line(x + w, y, x + w, y + h, size, color24);
}

void SSD1331_Triangle(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
		uint16_t x3, uint16_t y3, uint8_t size, uint32_t color24) {
	SSD1331_Line(x1, y1, x2, y2, size, color24);
	SSD1331_Line(x2, y2, x3, y3, size, color24);
	SSD1331_Line(x3, y3, x1, y1, size, color24);
}

void SSD1331_Triangle_Fill(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2,
	...
	for (curpixel = 0; curpixel <= numpixels; curpixel++) {
		SSD1331_Line(x, y, x3, y3, 1, color24);
...
void SSD1331_Ellipse(int16_t x0, int16_t y0, int16_t rx, int16_t ry,
		uint8_t fill, uint8_t size, uint32_t color24) {
	int16_t x, y;
	int32_t rx2 = rx * rx;
	int32_t ry2 = ry * ry;
	int32_t fx2 = 4 * rx2;
	...
	if (fill) {
		for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry);
				ry2 * x <= rx2 * y; x++) {
			SSD1331_Line(x0 - x, y0 - y, x0 + x + 1 - size, y0 - y, size,
					color24);
			SSD1331_Line(x0 - x, y0 + y, x0 + x + 1 - size, y0 + y, size,
					color24);
			if (s >= 0) {
		...
		for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry);
				ry2 * x <= rx2 * y; x++) {
			SSD1331_Rect_Fill(x0 + x, y0 + y, size, size, color24);
			SSD1331_Rect_Fill(x0 - x, y0 + y, size, size, color24);
			SSD1331_Rect_Fill(x0 + x, y0 - y, size, size, color24);
			SSD1331_Rect_Fill(x0 - x, y0 - y, size, size, color24);
			if (s >= 0) {
...
void SSD1331_Circle(uint16_t x, uint16_t y, uint8_t radius, uint8_t fill,
		uint8_t size, uint32_t color24) {
	int a_, b_, P;
	a_ = 0;
	b_ = radius;
	...
	while (a_ <= b_) {
		if (fill == 1) {
			SSD1331_Rect_Fill(x - a_, y - b_, 2 * a_ + 1, 2 * b_ + 1, color24);
			SSD1331_Rect_Fill(x - b_, y - a_, 2 * b_ + 1, 2 * a_ + 1, color24);
		} else {
			SSD1331_Rect_Fill(a_ + x, b_ + y, size, size, color24);
			SSD1331_Rect_Fill(b_ + x, a_ + y, size, size, color24);
			SSD1331_Rect_Fill(x - a_, b_ + y, size, size, color24);
			SSD1331_Rect_Fill(x - b_, a_ + y, size, size, color24);
			SSD1331_Rect_Fill(b_ + x, y - a_, size, size, color24);
			SSD1331_Rect_Fill(a_ + x, y - b_, size, size, color24);
			SSD1331_Rect_Fill(x - a_, y - b_, size, size, color24);
			SSD1331_Rect_Fill(x - b_, y - a_, size, size, color24);
		}
		if (P < 0) {
			P = (P + 3) + (2 * a_);
			a_++;
...
void LCD_Circle_Helper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername,
	...
	while (x < y) {
		...
		if (cornername & 0x4) {
			SSD1331_Rect_Fill(x0 + x, y0 + y, size, size, color24);
			SSD1331_Rect_Fill(x0 + y, y0 + x, size, size, color24);
		}
		if (cornername & 0x2) {
			SSD1331_Rect_Fill(x0 + x, y0 - y, size, size, color24);
			SSD1331_Rect_Fill(x0 + y, y0 - x, size, size, color24);
...
void SSD1331_Rect_Round(uint16_t x, uint16_t y, uint16_t length, uint16_t width,
		uint16_t r, uint8_t size, uint32_t color24) {
	SSD1331_Line(x + (r + 2), y, x + length + size - (r + 2), y, size, color24);
	SSD1331_Line(x + (r + 2), y + width - 1, x + length + size - (r + 2),
			y + width - 1, size, color24);
	SSD1331_Line(x, y + (r + 2), x, y + width - size - (r + 2), size, color24);
	SSD1331_Line(x + length - 1, y + (r + 2), x + length - 1,
...
void LCD_Circle_Fill_Helper(int16_t x0, int16_t y0, int16_t r,
	...
	while (x < y) {
		...
		if (cornername & 0x1) {
			SSD1331_Line(x0 + x, y0 - y, x0 + x, y0 - y + 2 * y + delta, 1,
					color24);
			SSD1331_Line(x0 + y, y0 - x, x0 + y, y0 - x + 2 * x + delta, 1,
					color24);
		}
		if (cornername & 0x2) {
			SSD1331_Line(x0 - x, y0 - y, x0 - x, y0 - y + 2 * y + delta, 1,
...
void SSD1331_Rect_Round_Fill(uint16_t x, uint16_t y, uint16_t length,
		uint16_t width, uint16_t r, uint32_t color24) {
	SSD1331_Rect_Fill(x + r, y, length - 2 * r, width, color24);
	LCD_Circle_Fill_Helper(x + length - r - 1, y + r, r, 1, width - 2 * r - 1,
			color24);
	LCD_Circle_Fill_Helper(x + r, y + r, r, 2, width - 2 * r - 1, color24);
}

static void LCD_Char(int16_t x, int16_t y, const SSD1331_GFXglyph *glyph,
		const SSD1331_GFXfont *font, uint8_t size, uint32_t color24) {
	...
	for (cur_y = 0; cur_y < glyph->height; cur_y++) {
		...
		if (set_pixels > 0) {
			SSD1331_Rect_Fill(x + (glyph->xOffset + cur_x - set_pixels) * size,
					y + (glyph->yOffset + cur_y) * size, size * set_pixels,
					size, color24);
			set_pixels = 0;
		}
...
void SSD1331_Font(uint16_t x, uint16_t y, char *text,
		const SSD1331_GFXfont *p_font, uint8_t size, uint32_t color24) {
	int16_t cursor_x = x;
	int16_t cursor_y = y;
	SSD1331_GFXfont font;
	memcpy(&font, p_font, sizeof(SSD1331_GFXfont));
	for (uint16_t text_pos = 0; text_pos < strlen(text); text_pos++) {
		...
		if (c == '\n') {
			...
			SSD1331_GFXglyph glyph;
			memcpy(&glyph, &font.glyph[c - font.first],
					sizeof(SSD1331_GFXglyph));
...
void SSD1331_IMG(const unsigned char *img, uint16_t x, uint16_t y,
	...
	if ((x + width > SSD1331_WIDTH) | (y + height > SSD1331_HEIGHT)) {
		return;
	}
	...
	for (yy = 0; yy < height; yy++) {
		...
		LCD_Command(x);
		LCD_Command(SSD1331_WIDTH - 1);
		//set row point
		LCD_Command(CMD_SET_ROW_ADDRESS);
		LCD_Command(y + yy);
		LCD_Command(SSD1331_HEIGHT - 1);
		HAL_GPIO_WritePin(SSD1331_DC_PORT, SSD1331_DC_PIN, GPIO_PIN_SET);
		HAL_GPIO_WritePin(SSD1331_CS_PORT, SSD1331_CS_PIN, GPIO_PIN_RESET);
...
void SSD1331_copyWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1,
...
void SSD1331_setDisplayMode(SSD1331_DisplayMode mode) {
	LCD_Command(mode);
}

void SSD1331_setDisplayPower(SSD1331_DisplayPower power) {
	LCD_Command(power);
}
