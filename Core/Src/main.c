...
/* USER CODE BEGIN PTD */
typedef enum {
	STATE_IDLE, STATE_PRESSED, STATE_HELD, STATE_RELEASE
} BUTTON_STATE_T;
...
static button_info_t encoder_a_contact = { .last_value = GPIO_PIN_SET, .state =
		STATE_IDLE, .button_state_recording = { GPIO_PIN_SET, GPIO_PIN_SET,
		GPIO_PIN_SET }, .recording_counter = 0 };
static button_info_t encoder_b_contact = { .last_value = GPIO_PIN_SET, .state =
		STATE_IDLE, .button_state_recording = { GPIO_PIN_SET, GPIO_PIN_SET,
		GPIO_PIN_SET }, .recording_counter = 0 };
static button_info_t encoder_click_contact = { .last_value = GPIO_PIN_SET,
		.state = STATE_IDLE, .button_state_recording = { GPIO_PIN_SET,
				GPIO_PIN_SET, GPIO_PIN_SET }, .recording_counter = 0 };
...
void button_info_update(GPIO_PinState current_value, button_info_t *button_info) {
	if (button_info->state == STATE_IDLE && current_value == GPIO_PIN_RESET
	...
	button_info->recording_counter < BUTTON_STATE_RECORDING_LEN) {
		button_info->button_state_recording[button_info->recording_counter] =
				current_value;
	...
	if (button_info->state == STATE_RELEASE
			&& button_info->recording_counter < 2) {
		button_info->recording_counter++;
	}
...
void ENC_Buttons_Read() {
	if (encoder_flag == 1) {
		button_info_update(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_0),
				&encoder_a_contact);
		button_info_update(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_1),
				&encoder_b_contact);
		button_info_update(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_2),
				&encoder_click_contact);
		...
		if (encoder_a_contact.state == STATE_RELEASE
				&& encoder_b_contact.state == STATE_HELD) {
			encoder_diff += 1 * encoder_multiplier;
		}

		if (encoder_b_contact.state == STATE_RELEASE
				&& encoder_a_contact.state == STATE_HELD) {
			encoder_diff += -1 * encoder_multiplier;
		}
